...
ManageBtrfs_DefineMountPoints() {
    ...
    # Defining operating system's "/boot" subvolume's name and mount point
    ...
    if [[ $(findmnt --mountpoint /boot --types btrfs) ]]
    then
    ...
    # Defining boot loader entries location
    if [[ $OSBOOTBTRFSSUBVOLUMEMOUNTPOINT ]]
    then
    ...
    elif [[ $OSBOOTBTRFSFOLDERMOUNTPOINT ]]
    then
